...
export class TransparencyCrawlAdsSource implements AdsSource {
  ...
  async getAdDetail(p: GetAdDetailParams): Promise<{ detail: AdDetail; apiCalls: number }> {
    ...
    // 이미지·텍스트: 상세(RPC) 후 CRAWL_DETAIL_WAIT_MS 만큼 대기하고 미리보기 수집.
    // 기본 0(대기 없음) — content.js 는 정적 텍스트라 대기가 수집 품질과 무관함이 실측 확인됨.
    const detailWaitMs = Number(process.env.CRAWL_DETAIL_WAIT_MS ?? 0);
