...
- **연속 수집 한도 + 자동 재개(2026-07-31 실측):** 502건 연속 상세 수집 시 봇 차단 → `maxPerRun`(팝업 "연속 한도", 기본 500)건마다 끊고 "재개 간격(분)"(기본 10분)만큼 쉬었다가 **자동 재개**해 완주한다(0=자동 재개 없이 중단). **백로그 방식:** 목록·신규판정 결과를 `chrome.storage`(`pendingWork`)에 저장 — 한도/차단으로 멈췄다 재개하거나 "수집 시작"을 다시 눌러도 **목록 재수집 없이 남은 상세부터** 이어서. 10건마다 백로그 갱신(탭 닫혀도 진행 보존), 비면 삭제 → 다음 수집은 새로 목록부터.
...
- `ADS_SOURCE` 환경변수로 **서버 측** 광고 데이터 소스를 고른다(브라우저 확장 경로와 별개). **롤백은 이 값만 변경**(코드 변경 없음):
  ...
  - `crawl` (무료·비공식·실험적): `TransparencyCrawlAdsSource`. 투명성 센터 내부 RPC 직접 호출(curl).
    ...
    - **⚠️ content.js 템플릿은 세 종류다(실측):** (1) `adData` JSON 템플릿(discover 등) — `\x27headline\x27: \x27…\x27` 형태 → `fieldValue` 로 추출. (2) **HTML 마크업 템플릿**(creativeType 46 이미지 레이아웃) — adData 없이 완성 HTML 이 직접 들어있다 → `componentsFromHtmlTemplate` 이 폴백: 문구=`title`/`body` 클래스 div 안의 `<a>` 텍스트(`<br>`→공백), CTA=`data-asoch-targets="…btnClk…"` 앵커, 랜딩=클릭 URL 의 `adurl=` 파라미터(percent-decode), 로고=정사각 소형(w=h≤200) `background-image` simgad, 배너=그 외 bg 이미지 중 최대 면적(쿼리 제거=원본 해상도). 이미지 광고 상당수가 이 템플릿이라 fieldValue 만으론 문구가 통째로 빈다. (4) **텍스트+로고 합성형(gpa) 템플릿** — adData 에 `headline`·**`body`**·**`callToAction`**·`logo`·`visible_url` 필드(설명·CTA 필드명이 다름, 실측). 배너는 **`squareImage`**(또는 marketingImage·**`carousel_0_image`**(캐러셀 첫 슬라이드) 등) 필드에 별도 simgad URL 로 온다 — 이 필드가 없는 하위 유형만 로고+문구 합성(이미지 없음이 정상). ⚠️ 첫 simgad 를 잡는 폴백은 로고를 집으므로 명시적 이미지 필드(`extractImageField`)를 먼저 본다. (4) **검색형 텍스트 광고("Single Ad Rendering Service")** — `AF_dataServiceRequests` 의 `"361903925"` 배열 `[ …null×7, headline, visibleUrl, description, … ]` → `componentsFromSearchAdTemplate`. **크리에이티브 이미지가 원래 없는 유형**(내장 data:image 는 별점 등 UI 아이콘 조각 — 이미지 미수집이 정상).
