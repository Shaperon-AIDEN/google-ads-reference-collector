...
- **스크린샷 캡처(조합 렌더링의 한계 보완):** 조합으로는 광고 단위의 비율·레이아웃을 완전 재현할 수 없어, 확장 "스크린샷 수집" 버튼이 광고 페이지 탭을 열어(활성 탭 필수 — `captureVisibleTab` 은 화면에 보이는 것만 캡처) 렌더 대기(실측 ~6초) 후 **대안 iframe 별로 캡처·크롭**(`_v<N>_<w>_<h>_` id 에서 idx·사이즈 파싱, devicePixelRatio 보정) → `POST /api/screenshot` → `ad_variations.screenshot`(bytea). 대시보드는 `/api/variations/[id]/screenshot` 로 스트리밍, 상세 "대안" 섹션에서 스크린샷 우선(없으면 조합 카드). 대상은 `GET /api/screenshot/targets`(이미지·텍스트 중 스크린샷 없는 것). 광고당 ~10초 소요.
...
- `ADS_SOURCE` 환경변수로 **서버 측** 광고 데이터 소스를 고른다(브라우저 확장 경로와 별개). **롤백은 이 값만 변경**(코드 변경 없음):
  ...
  - `crawl` (무료·비공식·실험적): `TransparencyCrawlAdsSource`. 투명성 센터 내부 RPC 직접 호출(curl).
    ...
    - **⚠️ content.js 템플릿은 두 종류다(실측):** (1) `adData` JSON 템플릿(discover 등) — `\x27headline\x27: \x27…\x27` 형태 → `fieldValue` 로 추출. (2) **HTML 마크업 템플릿**(creativeType 46 이미지 레이아웃) — adData 없이 완성 HTML 이 직접 들어있다 → `componentsFromHtmlTemplate` 이 폴백: 문구=`title`/`body` 클래스 div 안의 `<a>` 텍스트(`<br>`→공백), CTA=`data-asoch-targets="…btnClk…"` 앵커, 랜딩=클릭 URL 의 `adurl=` 파라미터(percent-decode), 로고=정사각 소형(w=h≤200) `background-image` simgad, 배너=그 외 bg 이미지 중 최대 면적(쿼리 제거=원본 해상도). 이미지 광고 상당수가 이 템플릿이라 fieldValue 만으론 문구가 통째로 빈다.
...
- **대량 크롤 차단 주의:** 광고 수백~수천 건을 크롤하면 상세 요청 폭주로 Google 봇 차단(HTML 응답 또는 `/sorry` 302) 발생. 완화책: 큐 `batchSize=2`, 크롤 전송 `CRAWL_THROTTLE_MS` 지연(지터로 값~2배). 차단은 **IP 레이트리밋**이 주원인이라 헤더 위조로는 못 피한다(TLS/HTTP2 지문은 curl 로 흉내 불가, 탐지 회피 도구는 도입 안 함). 차단 시 수십 분~수 시간 후 자동 해제 — 그 사이 재시도 금지(연장됨).
